    ...
    <section class="mContent_w">
      <div class="mModule mGoodBook_w">
        <div class="mGoodBookL">
          <h3 class="mModuleTi">{{ dataInfo.head.name }}</h3>
          <div class="mModuleTe">舍不得读完的好书</div>
          <router-link  :to="'/book?book_id='+dataInfo.head.book_list.book_id" class="mGoodBookLImg"><img :src="dataInfo.head.book_list.cover_img" :alt="dataInfo.head.book_list.name"/></router-link>
        </div>
        <div class="mGoodBookR">
          <h3 class="mModuleTi">{{ dataInfo.head.channel_a.channel_name }}</h3>
          <div class="mModuleTe"><router-link :to="'/classifylist?category_id='+dataInfo.head.channel_a.category_id">{{ dataInfo.head.channel_a.label }}</router-link></div>
          <router-link to="'/classifylist?category_id='+dataInfo.head.channel_a.category_id" class="mGoodBookBtn1">壮志豪言</router-link>
          <h3 class="mModuleTi mModuleTi1">女生频道</h3>
          <div class="mModuleTe"><router-link :to="'/classifylist?category_id='+dataInfo.head.channel_b.category_id">{{ dataInfo.head.channel_b.label }}</router-link></div>
          <router-link to="/classifylist?id=13" class="mGoodBookBtn2">言情细腻</router-link>
        </div>
      </div>
      <div class="mModule">
        <h3 class="mModuleTi">{{ dataInfo.first_channel.first_channel_data.category_name }}</h3>
        <ul class="mModuleList">
          <li v-for="list in dataInfo.first_channel.book_list" :key="list.id"><router-link :to="'/book?book_id='+list.book_id"><span><img :src="list.cover_img" :alt="list.name"/></span><p>{{ list.name }}</p></router-link></li>
        </ul>
      </div>
      <div class="mModule">
        <h3 class="mModuleTi">{{ dataInfo.second_channel.second_channel_data.category_name }}<router-link to="'/classifylist?id='+dataInfo.second_channel.second_channel_data.category_id">查看更多 &gt;</router-link></h3>
        <ul class="mModuleList">
          <li v-for="list in dataInfo.second_channel.book_list" :key="list.id"><router-link :to="'/book?book_id='+list.book_id"><span><img :src="list.cover_img" :alt="list.name"/></span><p>{{ list.name }}</p></router-link></li>
        </ul>
      </div>
      <div class="mModule">
        <h3 class="mModuleTi">{{ dataInfo.third_channel.second_channel_data.category_name }}<router-link to="'/classifylist?id='+dataInfo.third_channel.second_channel_data.category_id">查看更多 &gt;</router-link></h3>
        <ul class="mModuleLista">
          <li v-for="list in dataInfo.third_channel.book_list" :key="list.id">
